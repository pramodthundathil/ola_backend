...
class DeviceEnrollmentPagination(PageNumberPagination):
    page_size = 10
...
class DeviceEnrollmentAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        Create a new device enrollment and automatically initiate enrollment with Samsung KNOX or Equality.
        ...
        **Auto-populated fields:**
        ...
        - `locking_system`: Auto-determined (KNOX for Samsung, EQUALITY for others)
        - `enrollment_qr_code`: Generated by KNOX/Equality
        - `enrollment_link`: Generated by KNOX/Equality
    ...
    def _initiate_enrollment(self, enrollment, customer):
        """
        Initiate enrollment with KNOX or Equality based on device brand
        ...
        try:
            ...
            elif enrollment.locking_system == 'EQUALITY':
                # Equality enrollment (simulated for now, configured later)
                return {
                    'success': True,
                    'enrollment_id': f"EQ-{enrollment.imei}",
                    'qr_code': "https://ola-credit.equality.locking/qr-placeholder",
                    'enrollment_link': "https://ola-credit.equality.locking/link-placeholder",
                    'error': None
                }
    ...
    @swagger_auto_schema(
        ...
        **Query Parameters:**
        ...
        - `locking_system` (optional): Filter by locking system (KNOX, EQUALITY, NONE)
        ...
        try:
            ...
            if search:
                queryset = queryset.filter(
                    ...
                    Q(device_brand_name__icontains=search) |
                    Q(device_model__model_name__icontains=search)
                )
            ...
            # Pagination (only apply if 'page' query param is passed)
            page_param = request.query_params.get('page')
            if page_param:
                paginator = DeviceEnrollmentPagination()
                page = paginator.paginate_queryset(queryset, request)
                if page is not None:
                    serializer = DeviceEnrollmentSerializer(page, many=True)
                    logger.info(f"[DeviceEnrollment] Retrieved paginated enrollments")
    ...
    def patch(self, request, id):
        try:
            ...
            # Keep track if IMEI or locking system is changing to re-trigger enrollment
            imei_changed = 'imei' in request.data and request.data['imei'] != enrollment.imei
            locking_system_changed = 'locking_system' in request.data and request.data['locking_system'] != enrollment.locking_system
            ...
            if imei_changed or locking_system_changed:
                logger.info(f"[DeviceEnrollment] Re-initiating enrollment for updated device ID={id}")
                enrollment.refresh_from_db()
                enrollment_result = self._initiate_enrollment(enrollment, enrollment.customer)
                if enrollment_result['success']:
                    enrollment.enrollment_status = 'QR_GENERATED'
                    enrollment.enrollment_qr_code = enrollment_result.get('qr_code', '')
                    enrollment.enrollment_link = enrollment_result.get('enrollment_link', '')
    ...
    def _lock_device(self, enrollment):
        ...
        try:
            if enrollment.locking_system == 'KNOX':
                ...
                # Equality locking simulated
                return {
                    'success': True,
                    'message': 'Lock command simulated successfully (EQUALITY)'
                }
            else:
                return {
                    'success': False,
    ...
    def _unlock_device(self, enrollment):
        ...
        try:
            if enrollment.locking_system == 'KNOX':
                ...
                # Equality unlocking simulated
                return {
                    'success': True,
                    'message': 'Unlock command simulated successfully (EQUALITY)'
                }
            else:
                return {
                    'success': False,
